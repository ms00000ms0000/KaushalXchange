...
class Module6Fragment : Fragment() {
    ...
    private val questions = listOf(
        Question("Which mode opens a file for appending?", listOf("'r'", "'w'", "'a'", "'x'"), 2),
        Question("Which function reads entire file contents?", listOf("read()", "readline()", "readlines()", "open()"), 0),
        Question("Which statement automatically closes a file?", listOf("with open(...)", "open(...)", "file.close()", "close(file)"), 0)
    )
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.activity_module6_fragment, container, false)
    }
