...
class QuestionAdapter(
    ...
    private val isSubmitted: () -> Boolean   // <-- pass flag from activity
) : RecyclerView.Adapter<QuestionAdapter.QuestionViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): QuestionViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.assessment_questions, parent, false)
    ...
    override fun onBindViewHolder(holder: QuestionViewHolder, position: Int) {
        ...
        question.options.forEachIndexed { index, optionText ->
            val radioButton = RadioButton(holder.itemView.context).apply {
                ...
                // --- Highlight answers only if submitted ---
                if (isSubmitted()) {
                    isEnabled = false // disable after submit
                    when {
        ...
        // Save user answer before submission
